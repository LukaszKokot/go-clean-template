...
func (r defaultUserAvatarRepository) CreateOrUpdate(userID string, blob io.Reader) (*string, error) {
	// This is where we upload our blob and make sure it has a unique id so we
	// can override it if it already exists.

	// And we would return the public HTTPS location of the avatar if it works fine.
	return nil, nil
}
